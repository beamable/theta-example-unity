...
using Beamable.Microservices.ThetaFederation.Features.SolcWrapper.Exceptions;
using Beamable.Microservices.ThetaFederation.Features.SolcWrapper.Models;
using Newtonsoft.Json;

namespace Beamable.Microservices.ThetaFederation.Features.SolcWrapper
{
    public static class Solc
    {
        ...
        private const string ExecutableAmd64 = "solc-linux-amd64-v0.8.20";
        ...
        public static async Task<SolidityCompilerOutput> Compile(SolidityCompilerInput input)
        {
            ...
            BeamableLogger.Log("Compiling input: " + inputText);
            var inputFilePath = Path.GetTempFileName();

            await File.WriteAllTextAsync(inputFilePath, inputText);

            var solcBasePath = "--base-path=../Contracts";
            try
            {
                var outputText = await Execute(GetExecutable(), $"--standard-json {inputFilePath} {solcBasePath}", SolcDirectory);
                BeamableLogger.Log("outputText: " + outputText);
