...
namespace Beamable.Microservices.ThetaFederation.Features.Minting.Storage
{
    public class MintCollection : IService
    {
            ...
            if (_collection is null)
            {
                ...
                await _collection.Indexes.CreateManyAsync(new[]
                {
                    new CreateIndexModel<Mint>(
                        Builders<Mint>.IndexKeys.Ascending(x => x.ContractName).Ascending(x => x.ContentId)
                            .Ascending(x => x.TokenId).Ascending(x => x.TransactionHash),
                        new CreateIndexOptions { Unique = true }),
                    new CreateIndexModel<Mint>(Builders<Mint>.IndexKeys.Ascending(x => x.ContractName)
                        .Ascending(x => x.TokenId).Ascending(x => x.ContentId),
                        new CreateIndexOptions { Unique = true }),
                    new CreateIndexModel<Mint>(Builders<Mint>.IndexKeys.Ascending(x => x.ContractName)
                        .Ascending(x => x.InitialOwnerAddress).Ascending(x => x.ContentId)),
        ...
        public async Task<List<TokenIdMapping>> GetTokenMappingsForContent(string contractName,
            IEnumerable<string> contentIds)
        {
            var collection = await Get();
            var mints = await collection.Find(x => x.ContractName == contractName && contentIds.Contains(x.ContentId))
        ...
        public async Task BulkSaveMetadata(IList<UpdateMetadataRequest> requests)
        {
            ...
            var updates = requests.Select(request =>
                new UpdateManyModel<Mint>(Builders<Mint>.Filter.Eq(x => x.TokenId, request.TokenId),
                    Builders<Mint>.Update.Set(x => x.Metadata, request.Metadata))).ToList();
        ...
        public async Task<bool> TryInsertMint(Mint mint)
        {
            var collection = await Get();
            try
            {
